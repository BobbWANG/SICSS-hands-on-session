...
files <- c('/Users/bowang/Documents/workstation/SICSS-hands-on-session/companies_in_the_pilot_study/AL 1-1.html', '/Users/bowang/Documents/workstation/SICSS-hands-on-session/companies_in_the_pilot_study/AR 1-1.html')
...
```{r}
rawHTML <- read_html(x = "/companies_in_the_pilot_study/AL 1-1.html")
```


```{r}
Merge_P1 <- readxl::read_xlsx("60_descriptions_meta_data.xlsx",na = "",col_names = T, sheet = "Sheet1")
Merge_P2 <- readxl::read_xlsx("60_used_Descriptions.xlsx",na = "",col_names = T, sheet = "Sheet2")
```

```{r}
Merge_P1$Organizaiton[!(Merge_P1$Organizaiton %in% Merge_P2$Organizaiton)]
```
...
```{r}
Merged <- merge(Merge_P1, Merge_P2, by = "Organizaiton")
```
